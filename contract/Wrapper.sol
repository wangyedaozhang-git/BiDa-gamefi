...
contract NFTWrapper is TokenERC20{
	...
	function nftInfo(uint256 i) external view returns(uint256, uint256, uint256, uint256, uint256, uint256) {
		if(users[msg.sender].exists == 0) {
			return (0, 0, 0, 0, 0, 0);
		} else {
			return (users[msg.sender].nfts[i], users[msg.sender].nfts[i + 1], users[msg.sender].nfts[i + 2], 
				users[msg.sender].nfts[i + 3], users[msg.sender].nfts[i + 4], users[msg.sender].nfts[i + 5]);
		}
	}
}
